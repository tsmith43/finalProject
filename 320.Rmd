...
Looking at this graph below we can tell that the USA has the largest number of attacks compared to Australia and South Africa. But this huge difference could purely be based on the population differences between the 6 countries. So to combat this, we can look at the different graphs individually as they progress through time. In the graph below we can see the different graphs as time progresses. We can see these different graphs by utilizing the facet_grid function from the ggplot library. We notice with the first graph that is generally difficult to compare the 6 countries. To fix this, how about we change the scaling for each of the different countries. Changing this scaling helps us see the actual increase and change throughout time. We notice that with each country there is a positive correlation between time and the number of attacks. What could be causing this increase? It looks as though the USA has the strongest correlation, with the steepest slope. Let's talk more about why the USA might have this steep slope. 
```{r}
...
Let's see if there is a relationship between time and number of shark attacks for USA. We'll do that by fitting a linear regression model to it, and testing if the relationship is statistically significant.
```{r}
usa_data <- shark_data %>%
  filter(Country == "USA") %>%
  group_by(Year) %>%
...
```
We see that on average, number of shark attacks increases by 44.99% every year.
...
Let's visualize the linear regression by plotting the residuals vs years
```{r}
...
```
It seems that there is first a decreasing trend before 1970's which then becomes a steeply increasing trend thereafter. Let's look into what might be causing this trend.



5. Activities effect on Shark Attacks

As we can tell from above, we notice that time does have an impact on the number of attacks that we have in the country. We also noticed that there was a sharp decrease in the residuals which turned into a steep increasing trend after. The cause of this could potentially be based around the type of activities that were popular during these time frames. So the question we need to ask is, will the type of activity that you are doing cause an effect on the number of attacks caused? Does activity have a positive correlation to the number of incidents? Seen below is the graph for the top 6 Activities that have caused an attack in the USA. The highest number of attacks is found when someone is surfing in the USA. Let's dive deeper into the different types of attacks. 
```{r}
...
Similar to how we calculated the residuals above, we calculated the residuals based on the top 6 activities that we found within the USA. We did this by plotting the residuals just for the USA and then we colored those points based on the type of activity that caused that attack. We can notice that with this residual plot, as time progresses the different activities begin to have a lesser impact on the total number of attacks. Also, as time progresses we begin to see that surfing has become a very large issue in the USA. This makes sense, since our previous bar graph above says that surfing is the number one cause of sharrk attacks in the USA. So, looking at the residuals we can note that there is a strong relationship between surfing and the shark attacks in the USA.
```{r}
usa_data <- shark_data %>%
  filter(Country == "USA") %>%
...
We see that the trend is very similar to what we have with the USA residuals, let's break it down by country to see how each country contributes. Looking at the graph below, we notice that as a countries population increases, the number of surfing incidents also increase. Since the USA is the largest of these countries, it seems to have the strongest relationship through time with the number of surfing incidents. The graphs below show us that although the USA has a strong relationship, there is also a growing relationship with surfing throughout the other countries. 
...
Conclusion:
Being able to identify what has a strong relationship and cause with the number of shark attacks help people become aware of what to avoid and what to be careful of. As noted in the introduction, the article seems to be right with their analysis that sharks are beginning to come closer to the shore. With this increase in sharks at the shore we notice that shark attacks related to shore activities such as surfing and swimming are beginning to increase in a significant rate. We noticed above that time has a strong relationship with the increase in shark attacks, and we also noticed that surfing is also a big cause of shark attacks around the world. So no matter where you go, just remember that you can have fun surfing. But when you're surfing you need to be aware of the consequences that can be caused from doing it. 
