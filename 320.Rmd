...
```
At seems that there is first a decreasing trend before 1970's which then becomes a steeply increasing trend thereafter. Let's look into what might be causing this trend.
...
This graph shows us that throughout time, it seems as though swimming has been a consistent issue throughout time. We can tell that although Surfing is the largest issue in the USA, swimming has consistently been an issue throughout time. Even though the swimming has been consistent throughout the years, we can also tell from the bar graph below that surfing is still the number one activity that is done when someone is attacked. So this means that surfing must be an issue that has gotten more prevalent in recent years.
...
```
 ...
 We also notice while looking at the bar plot above that the total number of surfing attacks in the world is 952. This is very close to the number of surfing attacks in the USA which we found to be 559 attacks. Does this mean that surfing is as big of an issue in the world as it is in the USA?
...
We see that the trend is very similar to what we have with the USA residuals, let's break it down by country to see how each country contributes.
