...
face <- aus_shark_data %>%
  ...
  summarize(numOfAttacks = n()) %>%
  ggplot(aes(x=Year, y=numOfAttacks)) + geom_bar(stat = "identity", color = "black", fill = "navyblue") + facet_grid(Country ~ . ) + geom_smooth(method = lm, color = "pink")
